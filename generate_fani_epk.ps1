# generate_fani_epk.ps1 — Genera el EPK de FANI con imágenes embebidas en base64

$epkFolder  = "c:\Users\Ezequiel Zocco\Documents\Claude\Projects\DJ PRESSKITS\`u{1F4C1} EPK - [FANI]"
$ridersFolder = "c:\Users\Ezequiel Zocco\Documents\Claude\Projects\DJ PRESSKITS\RIDERS TECNICOS"
$outputFile = "$epkFolder\fani_epk.html"
...
Write-Host "Cargando imagenes principales..." -ForegroundColor Cyan
$heroUri = Get-DataUri "$epkFolder\01 - Fotos\01 - Foto Principal\fani-348.jpg"
$bioUri  = Get-DataUri "$epkFolder\01 - Fotos\01 - Foto Principal\fani-270.jpg"

Write-Host "Cargando galeria..." -ForegroundColor Cyan
$galFolder = "$epkFolder\01 - Fotos\06 - Galeria imagenes (Redes, en cabina, etc)"
$galFiles  = @(
...
$galUris = $galFiles | ForEach-Object { Get-DataUri "$galFolder\$_" }

Write-Host "Cargando rider equipos..." -ForegroundColor Cyan
$cdj3000Uri  = Get-DataUri "$ridersFolder\CDJ PIONEER 3000.png"
$cdj2000Uri  = Get-DataUri "$ridersFolder\PIONEER CDJ 2000 NXS2.png"
$mixerV10Uri = Get-DataUri "$ridersFolder\MIXER PIONEER DJM V10.png"
$xone96Uri   = Get-DataUri "$ridersFolder\MIXER ALLEN&HEATH XONE 96.png"
$xone92Uri   = Get-DataUri "$ridersFolder\MIXER ALLEN&HEATH XONE 92.png"

$galHtml = ($galUris | ForEach-Object {
    "<div class=`"gal-item reveal`"><img src=`"$_`" alt=`"FANI`" loading=`"lazy`"></div>"
}) -join "`n"
...
.hero-bg::after {
  content: '';
  position: absolute; inset: 0;
  background: linear-gradient(to top, rgba(9,13,28,.92) 0%, rgba(9,13,28,.3) 50%, rgba(9,13,28,.1) 100%);
}
...
.bio-text p {
  font-size: 1rem; line-height: 1.85; color: var(--mute);
  margin-bottom: 20px;
}
.bio-photo {
  position: relative;
...
.musica-embed iframe {
  width: 100%; border-radius: 0; border: 1px solid var(--line2);
}
...
.music-link-text strong {
  display: block; font-size: .85rem; letter-spacing: .08em; font-weight: 500;
  margin-bottom: 3px;
}
.music-link-text span {
  font-size: .7rem; letter-spacing: .15em; text-transform: uppercase; color: var(--mute);
}
.youtube-embed {
  grid-column: 1 / -1;
...
.rider-card {
  border: 1px solid var(--line2); padding: 28px 20px 20px;
...
.rider-spec-val {
  font-size: .85rem; color: var(--mute); line-height: 1.7;
}
@media (max-width: 768px) {
...
.contact-link {
  ...
  padding: 14px 28px; border: 1px solid var(--line2);
...
.contact-booking {
  margin-top: 60px; padding-top: 48px; border-top: 1px solid var(--line);
}
.contact-booking p {
...
<nav id="main-nav">
  ...
  <a class="nav-booking" href="mailto:booking@fanicrew.com">Booking</a>
</nav>
    ...
    <div class="musica-grid">
      ...
      <div class="musica-links reveal">
        <a class="music-link" href="https://soundcloud.com/fani-dj" target="_blank" rel="noopener">
          <div class="music-link-icon">☁️</div>
  ...
  <div class="sec">
    ...
    <div class="rider-specs reveal">
      <div class="rider-spec-item">
        ...
        <div class="rider-spec-val">4× Pioneer CDJ 2000 NXS2 o CDJ 3000</div>
      </div>
      <div class="rider-spec-item">
...
<section id="contacto">
  <div class="sec">
    ...
    <div class="contact-grid reveal">
      <a class="contact-link" href="https://www.instagram.com/fani.dj/" target="_blank" rel="noopener">
        <span>@fani.dj</span>
      </a>
      <a class="contact-link" href="https://www.instagram.com/fani.crew/" target="_blank" rel="noopener">
        <span>@fani.crew</span>
      </a>
      <a class="contact-link" href="https://www.instagram.com/fanicrew.agency/" target="_blank" rel="noopener">
        <span>@fanicrew.agency</span>
      </a>
      <a class="contact-link" href="https://open.spotify.com/intl-es/artist/6MDSJaDjt9IMUiAGch5JJ5" target="_blank" rel="noopener">
        <span>Spotify</span>
      </a>
      <a class="contact-link" href="https://www.beatport.com/es/artist/fani/628629" target="_blank" rel="noopener">
        <span>Beatport</span>
      </a>
    </div>
    <div class="contact-booking">
      <p>Booking</p>
      <a href="https://www.instagram.com/fanicrew.agency/">@fanicrew.agency</a>
    </div>
  </div>
</section>
...
// Cabinas compartidas cycling
(function() {
  const names = document.querySelectorAll('.cabinas-name');
  ...
  function next() {
    names[current].classList.remove('active');
  ...
  // Wait for fade-out (700ms transition) then switch
...
Write-Host "Escribiendo archivo..." -ForegroundColor Cyan
[System.IO.File]::WriteAllText($outputFile, $html, [System.Text.Encoding]::UTF8)

$size = (Get-Item $outputFile).Length / 1MB
Write-Host "EPK generado: $outputFile ($([Math]::Round($size,1)) MB)" -ForegroundColor Green
